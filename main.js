...
async function generateSummaryForFile(app, file, settings) {
    const content = await app.vault.read(file);
    if (!settings.openAiApiKey) {
        return fallbackSummary(content);
    }
    try {
        const response = await axios.post(`${settings.openAiApiBaseUrl}/chat/completions`, {
        ...
        new obsidian.Notice('Note summary failed; using fallback text.');
        return fallbackSummary(content);
    }
}
async function saveSummary(app, content, summariesFolder, baseName) {
    const folder = obsidian.normalizePath(summariesFolder || 'Summaries');
    await ensureFolderExists$1(app, folder);
    const targetPath = obsidian.normalizePath(`${folder}/${baseName}-summary.md`);
...
async function generateMeetingSummary(transcript, settings, hasTimestamps) {
    if (!settings.autoSummarise)
        return null;
    ...
    try {
        ...
        new obsidian.Notice('Meeting summary generation failed; transcript saved without AI summary.');
        return null;
    }
}
async function ensureFolderExists$1(app, folder) {
...
class RecordingManager {
    ...
    async startRecording(parentFile) {
        if (this.session) {
            new obsidian.Notice('Recording already in progress.');
            return;
        }
        try {
            ...
            new obsidian.Notice('Recording started. Click the mic again or run stop command to finish.');
        }
        catch (error) {
            console.error('Failed to start recording', error);
            new obsidian.Notice('Unable to access microphone. Please check permissions.');
            this.cleanupStream();
        }
    }
    async stopRecording() {
        if (!this.session || !this.mediaRecorder) {
            ...
            new obsidian.Notice('No recording is currently running.');
            return;
        }
        ...
        try {
            ...
            new obsidian.Notice('Could not save audio file; continuing with transcription.');
        }
        const transcription = await this.transcribeAudio(audioBlob);
        if (!transcription) {
            new obsidian.Notice('Transcription failed or skipped. Audio saved if possible.');
            return;
        }
        ...
        new obsidian.Notice('Recording complete. Transcription note created.');
    }
    collectAudioBlob() {
    ...
    pauseRecording() {
        if (!this.mediaRecorder || !this.session || this.session.isPaused)
            return;
        try {
    ...
    async saveAudioFile(blob, startedAt) {
        const settings = this.getSettings();
        const folder = obsidian.normalizePath(settings.audioFolderPath || 'Audio');
        ...
        const path = await uniquePath(this.app, obsidian.normalizePath(`${folder}/${fileName}`));
    ...
    async transcribeAudio(blob) {
        ...
        if (!apiKey) {
            new obsidian.Notice('AssemblyAI API key missing; skipping transcription.');
            return null;
        }
        try {
            ...
            new obsidian.Notice(`Transcription failed: ${data?.error || status || 'see console for details'}`);
            return null;
        }
    }
    async createTranscriptionNote(transcript, parentFile, audioFilePath, startedAt) {
        const settings = this.getSettings();
        const folder = obsidian.normalizePath(settings.transcriptionFolderPath || 'Transcriptions');
        await ensureFolderExists(this.app, folder);
        ...
        const notePath = await uniquePath(this.app, obsidian.normalizePath(`${folder}/${sanitizedName}`));
    ...
    async appendLinkToParent(parentFile, transcriptionFile) {
    ...
    async appendMeetingSummary(noteFile, transcript) {
        const settings = this.getSettings();
        ...
        if (content.includes('## AI Meeting Summary'))
            return;
        const updated = `${content}\n\n## AI Meeting Summary\n\n${summary}\n`;
        await this.app.vault.modify(noteFile, updated);
    }
}
async function uploadAudio(blob, apiKey) {
    const arrayBuffer = await blob.arrayBuffer();
...
async function requestTranscript(uploadUrl, settings) {
    ...
    const response = await axios.post('https://api.assemblyai.com/v2/transcript', body, {
        headers: {
...
class RecordingStatusPanel {
    ...
    refreshState(paused = this.isPausedFn()) {
...
function buildFrontmatter(parentFile, audioFilePath, startedAt) {
...
async function uniquePath(app, initialPath) {
...
class ObsidianAISettingTab extends obsidian.PluginSettingTab {
    ...
    display() {
        ...
        this.addTextSetting(containerEl, 'Audio folder path', 'Folder in this vault where audio recordings are stored', 'audioFolderPath', 'Audio');
        this.addTextSetting(containerEl, 'Transcription folder path', 'Folder where transcription notes will be created', 'transcriptionFolderPath', 'Transcriptions');
    ...
    addTextSetting(containerEl, name, desc, key, placeholder) {
        new obsidian.Setting(containerEl)
            .setName(name)
            .setDesc(desc)
            .addText((text) => text
...
class ObsidianAiPlugin extends obsidian.Plugin {
    async onload() {
        ...
        this.registerEvent(this.app.workspace.on('file-menu', (menu, file) => {
            ...
            menu.addItem((item) => {
                item
                    ...
                    new obsidian.Notice('Summary saved.');
                });
            });
            menu.addItem((item) => {
                item
    ...
    async startRecordingForActiveFile() {
        const activeFile = this.app.workspace.getActiveFile();
        if (!activeFile) {
            new obsidian.Notice('No active file. Open a note before starting a recording.');
            return;
        }
        if (this.recordingManager.isRecording) {
            new obsidian.Notice('Recording already in progress.');
...
module.exports = ObsidianAiPlugin;
